...
typedef ComputeHitSlop = double Function(PointerDeviceKind kind, DeviceGestureSettings? settings);

class TableGestureRecognizer extends MyDragGestureRecognizer {
  /// Create a gesture recognizer for tracking movement on a plane.
  TableGestureRecognizer({Object? debugOwner, Set<PointerDeviceKind>? supportedDevices})
  ...
  bool hasSufficientGlobalDistanceToAccept(PointerDeviceKind pointerDeviceKind) {
    return _globalDistanceMoved.abs() > hitSlope(pointerDeviceKind, gestureSettings);
  }
  ...
  Velocity getPrimaryVelocity(Velocity value) {
    switch (dragDirection) {
      case TableScrollDirection.horizontal:
        return Velocity(pixelsPerSecond: Offset(value.pixelsPerSecond.dx, 0.0));
      case TableScrollDirection.vertical:
        return Velocity(pixelsPerSecond: Offset(0.0, value.pixelsPerSecond.dy));
      default:
        return value;
    }
  }
...
abstract class MyDragGestureRecognizer extends OneSequenceGestureRecognizer {
  ...
  GestureTableDragUpdateCallback? onUpdate;
  ...
  GestureTableDragEndCallback? onEnd;
  ...
  GestureDragDirection? selectionDragDirection;

  TableScrollDirection dragDirection = TableScrollDirection.unknown;
  ...
  ///
  /// To estimate the velocity of a gesture, [MyDragGestureRecognizer] calls
    ...
    if (event is PointerMoveEvent) {
      ...
      if (_state == _DragState.accepted) {
        _checkUpdate(
          ...
          delta: getDeltaForDetails(event.localDelta),
          primaryDelta: getPrimaryValueFromOffset(event.localDelta),
        ...
        if (hasSufficientGlobalDistanceToAccept(event.kind)) resolve(GestureDisposition.accepted);
      }
    }
  ...
  void _giveUpPointer(int pointer, {bool reject = true}) {
    ...
    // print('giveuppointer reject $reject');
    if (reject) {
  ...
  void _checkDown() {
    ...
    if (selectionDragDirection != null) {
      dragDirection = selectionDragDirection!(details);
    }
    assert(dragDirection != TableScrollDirection.unknown,
        'The dragDirection is unknown, use the function selectionDragDirection or set dragDirection with instance');
  }
  ...
  void _checkUpdate({
    ...
    assert(_initialButtons == kPrimaryButton);
    final TableDragUpdateDetails details = TableDragUpdateDetails(
  ...
  void _checkEnd(int pointer) {
    ...
    if (estimate != null && isFlingGesture(estimate, tracker.kind)) {
      ...
      final primaryVelocity = getPrimaryVelocity(velocity);
      details = TableDragEndDetails(
        velocity: primaryVelocity,
        xVelocity: primaryVelocity.pixelsPerSecond.dx,
        yVelocity: primaryVelocity.pixelsPerSecond.dy,
        xyVelocity: primaryVelocity.pixelsPerSecond.distance,
      );
      debugReport = () {
