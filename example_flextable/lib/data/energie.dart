...
class EnergieWarmte {
  ...
  TableModel makeTable({TargetPlatform? platform}) {
    ...
    for (List<_Item> rowItems in columnHeader) {
      ...
      for (_Item item in rowItems) {
        dataTable.addCell(
            ...
            column: columnTemp,
            cell:
                Cell(value: item.value, attr: {'background': Colors.grey[50]!}),
            columns: item.length);
    ...
    for (var item in rowHeader1) {
      dataTable.addCell(
          row: rowTemp,
          column: column,
          cell: Cell(
              value: item.value,
              attr: {'background': Colors.blueGrey[50]!, 'rotate': -90}),
          rows: item.length);
      rowTemp += item.length;
    }
    ...
    for (String value in rowHeader2) {
      dataTable.addCell(
        row: rowTemp,
        column: column,
        cell: Cell(
    ...
    for (int r = 0; r < dataRows; r++) {
      int dataColumns = data[r].length;

      if (endTableColumn < column + dataColumns) {
        endTableColumn = column + dataColumns;
      }
      ...
      for (int c = 0; c < dataColumns; c++) {
        ...
        Map attr = {};
        if (c == 3 && value is num) {
          attr['percentagBackground'] = PercentageBackground(
    ...
    h
      ..addList(
          ...
          endLevelOneIndex: 2,
          lineList: secondHeaderColumn)
      ..addList(startLevelOneIndex: 3, lineList: secondHorizontalMiddle)
      ..addList(startLevelOneIndex: 16, lineList: secondHorizontalMiddle)
      ..addList(startLevelOneIndex: 29, lineList: secondHorizontalMiddle)
      ..addList(startLevelOneIndex: 44, lineList: secondHorizontalMiddle);
