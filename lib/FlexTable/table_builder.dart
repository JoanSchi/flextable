...
abstract class TableBuilder {
  ...
  Widget? buildHeaderIndex(
      double tableScale, double headerScale, TableHeaderIndex tableHeaderIndex);
...
class DefaultTableBuilder extends TableBuilder {
  ...
  Widget? buildHeaderIndex(double tableScale, double headerScale,
    ...
    if (tableHeaderIndex.panelIndex <= 3 || tableHeaderIndex.panelIndex >= 12) {
      return Center(
        ...
        textScaleFactor: (tableScale < headerScale) ? tableScale : headerScale,
      ));
    } else {
      return Center(
          child: Text(
  ...
  LineHeader lineHeader(int panelIndex) {
    final tpli = _tableModel.layoutIndex(panelIndex);
...
class PaintSplitLines {
  ...
  PaintSplitLines(
      {this.freezeColors = const [
        const Color(0xFF016baa),
        const Color(0xFF015e96),
        const Color(0xFF004d7c)
      ],
  ...
  draw(PaintingContext context, Offset offset, Size size) {
    final w = tableModel.widthLayoutList;
    final h = tableModel.heightLayoutList;
    ...
    final xSplitState =
        w[2].inUse ? tableModel.stateSplitX : SplitState.NO_SPLITE;
    final ySplitState =
        h[2].inUse ? tableModel.stateSplitY : SplitState.NO_SPLITE;

    equalSplit = xSplitState == ySplitState;

    if (xSplitState == SplitState.SPLIT) {
      drawHorizontalSplit(canvas, ySplitState);
      drawVerticalSplit(canvas, xSplitState);
    } else {
      drawVerticalSplit(canvas, xSplitState);
      drawHorizontalSplit(canvas, ySplitState);
    }
  ...
  drawHorizontalSplit(Canvas canvas, SplitState splitState) {
    switch (splitState) {
      case SplitState.AUTO_FREEZE_SPLIT:
      case SplitState.FREEZE_SPLIT:
        {
          final freezeArea = tableModel.autoFreezeAreaY;
  ...
  drawVerticalSplit(Canvas canvas, SplitState splitState) {
    switch (splitState) {
      case SplitState.AUTO_FREEZE_SPLIT:
      case SplitState.FREEZE_SPLIT:
        {
          final freezeArea = tableModel.autoFreezeAreaX;
          if (freezeArea.freeze) {
...
class PercentageBackground {
  ...
  bool operator ==(Object o) {
    ...
    return o is PercentageBackground &&
