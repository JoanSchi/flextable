...
/// The possible states of a [TableScaleGestureRecognizer].
enum _ScaleState {
...
///
/// [TableScaleGestureRecognizer] tracks the pointers in contact with the screen and
...
class TableScaleGestureRecognizer extends OneSequenceGestureRecognizer {
  ...
  void _advanceStateMachine(bool shouldStartIfAccepted, PointerDeviceKind pointerDeviceKind) {
    ...
    if (_state == _ScaleState.possible) {
      ...
      // print(
      //     'state machine spanDelta $spanDelta focalPointDelta $focalPointDelta ${computeScaleSlop(pointerDeviceKind)}  ${computePanSlop(pointerDeviceKind)}');
      // if (spanDelta > computeScaleSlop(pointerDeviceKind) ||
      //     focalPointDelta > computePanSlop(pointerDeviceKind))
      if (_pointerLocations.keys.length > 1) resolve(GestureDisposition.accepted);
    }
