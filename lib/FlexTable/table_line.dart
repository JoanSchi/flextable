...
typedef Merge = LineLinkedListEntry Function(LineLinkedListEntry link, LineLinkedListEntry merge);
...
class TableLineList extends LineLinkedList<TableLineNode> {
  ...
  addLineNode(
    ...
    add(
        TableLineNode(
            startIndex: startLevelOneIndex is int ? requestNewIndex(startLevelOneIndex) : startLevelOneIndex,
            endIndex: endLevelOneIndex == null
                ? null
                : (endLevelOneIndex is int ? requestNewIndex(endLevelOneIndex) : endLevelOneIndex),
            lineList: LineNodeList(lineNode: lineNode, requestNewIndex: requestLevelTwoIndex)),
        merge,
        addEmpty);
  }
  ...
  TableLineNode merge(LineLinkedListEntry link, LineLinkedListEntry merge) {
    ...
    return TableLineNode(lineList: list, startIndex: merge.startIndex, endIndex: merge.endIndex);
  }

  LineNodeList createLineNodeList({LineNode? lineNode, var startLevelTwoIndex, var endLevelTwoIndex}) {
    return LineNodeList(
        requestNewIndex: requestLevelTwoIndex,
        lineNode: lineNode,
...
class TableLineNode extends LineLinkedListEntry<TableLineNode> {
  ...
  @override
  bool goToNext(entry) {
    return endIndex < entry.startIndex && (next != null && next!.startIndex <= entry.startIndex);
  }

  @override
  bool goToPrevious(entry) {
    return entry.endIndex < startIndex || (previous != null && entry.startIndex <= previous!.endIndex);
  }
  ...
  bool operator ==(Object o) {
    ...
    return o is TableLineNode && o.lineList == lineList;
  }
  ...
  @override
  TableLineNode copy() => TableLineNode(startIndex: startIndex, endIndex: endIndex, lineList: lineList.copy());
  ...
  @override
  String toString() => 'Fy(lineList: startIndex $startIndex endIndex $endIndex)';
}
...
class LineNodeList extends LineLinkedList<LineNode> {
  LineNodeList(
      {LineNode? lineNode, var startLevelTwoIndex, var endLevelTwoIndex, required RequestNewIndex requestNewIndex})
  ...
  addLineNode({required LineNode lineNode, var startLevelTwoIndex, var endLevelTwoIndex, addEmpty = false}) {
    if (startLevelTwoIndex != null) {
      setIndex(lineNode, startLevelTwoIndex, endLevelTwoIndex);
    }
    if (_length == 0) {
...
abstract class LineLinkedList<E extends LineLinkedListEntry<E>> {
  ...
  add(E link, Merge merge, bool addEmpty) {
    ...
    assert(link.startIndex <= link.endIndex, 'StartIndex is not smaller or equal compared to endIndex');
    ...
    if (_length == 0) {
      ...
      if (link.endIndex < _first!.startIndex || last!.endIndex < link.startIndex) {
        if (check) {
          print('0: found $found');
          print('0: ln $link');
        }
        _addOutside(link, found, addEmpty);
      } else if (found.startIndex <= link.startIndex && found.endIndex >= link.endIndex) {
        ...
        while (true) {
          ...
          if (found.startIndex > link.startIndex) {
            ...
            if (link.endIndex < found.startIndex) {
              ...
              final newFound =
                  _addOutside(link..endIndex = requestNewIndex(startFound.index - 1), found.previous, addEmpty);
          ...
          if (found.endIndex < link.endIndex) {
            ...
            // assert(found != null);
            found = _addInside(link..endIndex = endFound, found, merge, addEmpty);
            ...
            assert(link.startIndex <= link.endIndex, 'start: ${link.startIndex} end: ${link.endIndex}');

            if (check) {
              print('2: found after $found');
            }
    ...
    if (found == null || link.endIndex < found.startIndex) {
      if (link.isNotEmpty || addEmpty) {
        if (link.equalInterceptAndJoined(_first)) {
          ...
          assert(link.next == null || link.endIndex < link.next!.startIndex, 'Overlap is not alought');
          newFound = link;
        }
      }
      ...
      print('outside: should not happen li: $link');
      print('outside: should not happen found: $found');
    }
  ...
  E _addInside(E link, E? found, Merge merge, bool addEmpty) {
    ...
    if (found == null) {
      ...
      if (adjusted.differentIntercept(found)) {
        if (adjusted.startIndex > found.startIndex && adjusted.endIndex < found.endIndex) {
          ...
          assert(adjusted.endIndex <= found.endIndex, 'EndIndex out of bound: ${adjusted.endIndex}, startIndex equal');
  ...
  setIndex(E link, var startLevelIndex, var endLevelIndex) {
    assert(startLevelIndex != null, 'StartIndex can not be null, Where does the list or LineNode start?');
    link.setIndex(
        startIndex: startLevelIndex is int ? requestNewIndex(startLevelIndex) : startLevelIndex,
        endIndex:
            endLevelIndex == null ? null : (endLevelIndex is int ? requestNewIndex(endLevelIndex) : endLevelIndex));
  }
  ...
  bool operator ==(Object o) {
    ...
    if (o is LineLinkedList<E> && o._length == _length) {
      var element = _first;
      var elementObject = o._first;
...
class LineNode extends LineLinkedListEntry<LineNode> {
  ...
  copyObject(LineNode lineNode) {
    return LineNode(
      left: identical(lineNode.left, _emptyLine) ? left : left.copyObject(o: lineNode.left),
      top: identical(lineNode.top, _emptyLine) ? top : top.copyObject(o: lineNode.top),
      right: identical(lineNode.right, _emptyLine) ? right : right.copyObject(o: lineNode.right),
      bottom: identical(lineNode.bottom, _emptyLine) ? bottom : bottom.copyObject(o: lineNode.bottom),
      startIndex: lineNode.startIndex,
      endIndex: lineNode.endIndex,
    );
  }

  LineNode mergeLineNode({required LineNode lineIntercept, keepNoLine = false}) {
  ...
  @override
  bool get isNotEmpty {
    return left != gridLine || top != gridLine || right != gridLine || bottom != gridLine;
  }
  ...
  bool equalLink(LineNode? o) {
    ...
    return o != null && o.left == left && o.top == top && o.right == right && o.bottom == bottom;
  }
...
class _EmptyLine extends Line {
  const _EmptyLine() : super(line: TableLineOptions.NO, width: 0, color: const Color(0xffffffff), drawLine: false);
}
...
class Line {
  ...
  widthScaled(double scale) =>
      width * (scale < lowestScale ? lowestScale : (scale > highestScale ? highestScale : scale));
  ...
  bool operator ==(Object o) {
    ...
    return o is Line &&
        ...
        o.width == width &&
        o.color == color &&
        o._drawLine == _drawLine &&
        o.lowestScale == lowestScale &&
        o.highestScale == highestScale;
  }
...
abstract class LineLinkedListEntry<E extends LineLinkedListEntry<E>> {
  ...
  void setIndex({required Index startIndex, Index? endIndex}) {
    ...
    this.startIndex = startIndex;
    this.endIndex = endIndex ?? startIndex;
    assert(this.startIndex <= this.endIndex, 'EndIndex is smaller than startIndex!');
  }
...
printListNext(LineNodeList lineList) {
  ...
  while (t != null) {
    print('next: ${t.startIndex} ${t.endIndex}');
    t = t.next;
  }
}
...
printListPrevious(LineNodeList lineList) {
  ...
  while (t != null) {
    print('previous: ${t.startIndex} ${t.endIndex}');
    t = t.previous;
  }
}
